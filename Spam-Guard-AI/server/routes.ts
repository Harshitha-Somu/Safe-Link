...
export async function registerRoutes(
  ...
  app.post(api.scans.create.path, async (req: Request, res: Response) => {
    try {
      const input = insertScanSchema.parse(req.body) as { url: string };
  ...
  return res.status(400).json({ message: "Invalid URL format" });
      ...
      const analysis: any = await analyzeUrl(input.url);
  ...
  app.get(api.scans.get.path, async (req: Request, res: Response) => {
  ...
  app.get(api.scans.list.path, async(req: Request, res: Response) => {
